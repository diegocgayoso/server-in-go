...
const markMessageAsAnwsered = `-- name: MarkMessageAsAnwsered :exec
UPDATE messages
SET 
    answered = true
WHERE
...
func (q *Queries) MarkMessageAsAnwsered(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.Exec(ctx, markMessageAsAnwsered, id)
	return err
}
